...
COPY ./services/http_api_svr/go.mod ./services/http_api_svr/go.sum /app/services/http_api_svr/
...
RUN CGO_ENABLED=0 GOOS=linux go build -o /app/bin/http_api_svr.bin

# run the app
CMD ["/app/bin/http_api_svr.bin"]